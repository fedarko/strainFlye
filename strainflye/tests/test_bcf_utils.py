...
def test_parse_sf_bcf_good():
    ...
    with write_indexed_bcf(
        ...
        # This part of things -- parsing the BCF -- pysam handles, so we don't
        # go too in depth. Just verify it gets the positions, alleles, and
        # info right.
        ...
        for ri, rec in enumerate(bcf_obj.fetch()):
            ...
            # this part we'll have to change if we start calling multiple
            # mutations at a single position
            assert len(rec.alts) == 1
            assert rec.alts[0] == correct_alt[ri]

            # AAD is treated as a tuple containing one element by pysam, not
            # as a single value. This is because I've labelled the AAD INFO
            # field with a Number of "A", meaning that there is one of these
            # per alternate allele. Currently we don't call more than one
            # mutation at a position, so this doesn't do anything, but -- in
            # case we modify this in the future -- this leaves the door open
            # for us to add this in.
...
def test_parse_sf_bcf_missing_info_fields():
    ...
    with write_indexed_bcf(
        ...
        # file, so we have to keep recreating these expected error messages.
        # eesh.)
        assert str(ei.value) == (
            f"BCF file {fp} needs to have MDP and AAD info fields."
        )
...
def test_parse_sf_bcf_no_contigs_in_header():
    ...
    with write_vcf(vcf_text) as fh:
        vcf_fp = fh.name
        with pytest.raises(ParameterError) as ei:
            bu.parse_sf_bcf(vcf_fp)
        # yeah yeah i know the error message says BCF instead of VCF but we
        # assume that these files are all BCF, like literally the parameter
        # is --bcf or whatever, not worth saying "BCF or VCF" esp when we
        # assume that everything is indexed BCF, this particular issue just
        # comes up in testing (... so, this check is kind of useless since it
        # seems impossible to come up in practice, but whatever).
        assert str(ei.value) == (
...
def test_get_mutated_position_details_in_contig_lowercase():
    ...
    for middle in ("G\tC", "G\tc", "g\tC", "g\tc"):
        with write_arb_bcf(f"edge_1\t1234\t.\t{middle}\t.\t.\t.\n") as fh:
            bcf_obj = pysam.VariantFile(fh.name)
...
def test_verify_bcf_simple_multiallelic_sep_lines():
    with write_arb_bcf(
        ...
        assert str(ei.value) == (
            ...
            "position 43 on contig edge_1. strainFlye does not currently "
            "support BCF files containing multi-allelic mutations, sorry."
        )
...
def test_verify_bcf_simple_monomorphic_reference():
    ...
    with write_arb_bcf("edge_1\t45\t.\tA\t.\t.\t.\t.\n") as fh:
        fp = fh.name
        bcf_obj = pysam.VariantFile(fp)
        with pytest.raises(ParameterError) as ei:
            bu.verify_bcf_simple(bcf_obj, fp)
        assert str(ei.value) == (
            ...
            "position 45 on contig edge_1. strainFlye does not currently "
            "support BCF files containing these sorts of records, sorry."
        )
...
def test_verify_bcf_simple_indels():
    ...
    with write_arb_bcf("edge_1\t45\t.\tGTC\tG\t.\t.\t.\n") as fh:
        bcf_obj = pysam.VariantFile(fh.name)
        with pytest.raises(ParameterError) as ei:
            bu.verify_bcf_simple(bcf_obj, fh.name)
        assert str(ei.value) == f"BCF file {fh.name} " + exp_err_msg_suffix

    # simple deletion: based on vcf spec example in section 5.2.3
    with write_arb_bcf("edge_1\t45\t.\tTCG\tT\t.\t.\t.\n") as fh:
        bcf_obj = pysam.VariantFile(fh.name)
        with pytest.raises(ParameterError) as ei:
            bu.verify_bcf_simple(bcf_obj, fh.name)
        assert str(ei.value) == f"BCF file {fh.name} " + exp_err_msg_suffix
...
def test_verify_bcf_simple_symbolic_alt():
    ...
    for salt in (
        ...
        with write_arb_bcf(f"edge_1\t1000\t.\tT\t{salt}\t.\t.\t.\n") as fh:
            fp = fh.name
            bcf_obj = pysam.VariantFile(fp)
            with pytest.raises(ParameterError) as ei:
                bu.verify_bcf_simple(bcf_obj, fp)
...
def test_verify_bcf_simple_degen():
    with write_arb_bcf("edge_1\t45\t.\tG\tM\t.\t.\t.\n") as fh:
        fp = fh.name
        bcf_obj = pysam.VariantFile(fp)
        with pytest.raises(ParameterError) as ei:
            bu.verify_bcf_simple(bcf_obj, fp)
        assert str(ei.value) == (
            ...
            "nucleotides or other complex types of reference / alternate "
            "alleles, sorry."
        )

    with write_arb_bcf("edge_1\t1\t.\tN\tA\t.\t.\t.\n") as fh:
        fp = fh.name
        bcf_obj = pysam.VariantFile(fp)
        with pytest.raises(ParameterError) as ei:
            bu.verify_bcf_simple(bcf_obj, fp)
    ...
    with write_arb_bcf("edge_1\t10000000\t.\tN\tN\t.\t.\t.\n") as fh:
        fp = fh.name
        bcf_obj = pysam.VariantFile(fp)
        with pytest.raises(ParameterError) as ei:
            bu.verify_bcf_simple(bcf_obj, fp)
        assert str(ei.value) == (
            f"BCF file {fp} has a record at (1-indexed) position 10,000,000 "
            "on contig edge_1 where the reference (N) and/or alternate (N) "
            "are not in {A, C, G, T}. strainFlye does not support degenerate "
            "nucleotides or other complex types of reference / alternate "
            "alleles, sorry."
        )
...
def test_verify_bcf_simple_deletion_chars():
    for ref, alt in (("G", "*"), ("*", "G"), ("*", "*")):
        with write_arb_bcf(f"edge_1\t2022\t.\t{ref}\t{alt}\t.\t.\t.\n") as fh:
            fp = fh.name
            bcf_obj = pysam.VariantFile(fp)
            with pytest.raises(ParameterError) as ei:
                bu.verify_bcf_simple(bcf_obj, fp)
            assert str(ei.value) == (
                ...
                f"({alt}) are not in {{A, C, G, T}}. strainFlye does not "
                "support degenerate nucleotides or other complex types of "
                "reference / alternate alleles, sorry."
            )


def test_verify_bcf_simple_lowercase_ok():
    with write_arb_bcf("edge_1\t1234\t.\tG\tc\t.\t.\t.\n") as fh:
        bcf_obj = pysam.VariantFile(fh.name)
        bu.verify_bcf_simple(bcf_obj, fh.name)
    ...
    for middle in ("G\tG", "G\tg", "g\tG", "g\tg"):
        with write_arb_bcf(f"edge_1\t1234\t.\t{middle}\t.\t.\t.\n") as fh:
            ...
            assert str(ei.value) == (
                ...
                "contig edge_1 where the reference (G) matches the alternate "
                "(G). strainFlye does not currently support BCF files "
                "containing these sorts of records, sorry."
            )
...
def test_verify_bcf_simple_breakends():
    ...
    for bnd in ("T[edge_1:5000[", "]edge_1:5000]T", "T[edge_1:5000]", "T."):
        with write_arb_bcf(f"edge_1\t1000\t.\tT\t{bnd}\t.\t.\t.\n") as fh:
            fp = fh.name
            bcf_obj = pysam.VariantFile(fp)
            with pytest.raises(ParameterError) as ei:
                bu.verify_bcf_simple(bcf_obj, fp)
