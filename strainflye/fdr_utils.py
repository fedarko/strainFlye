...
def run_fix(bcf, fdr_info, num_info, fdr, output_bcf, fancylog):
    ...
    num_info: str
        Filepath to a TSV file describing numbers of mutations per megabase for
        the target contigs.
    ...
    ------
    ParameterError
        - If the indices and columns of the fdr_info and num_info files
    """
    ...
    # Load the estimated FDR and # mutations / mb files. Double-check that they
    # have the same index and columns.
    fi = pd.read_csv(fdr_info, sep="\t", index_col=0)
    ni = pd.read_csv(num_info, sep="\t", index_col=0)
    if (fi.index != ni.index).any() or (fi.columns != ni.columns).any():
        raise ParameterError(
            f"{fdr_info} and {num_info} should have the same rows and columns."
        )

    tsv_desc = "the FDR and # mutations / megabase TSV files"
    ...
    # not in the TSV files. This must be the decoy contig.
